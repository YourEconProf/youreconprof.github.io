...
```{r}
#| label: participation
#| fig-cap: "Labor Force Participation"
#| include: true
#| echo: false
#| warning: false
#| fig-height: 2.5
...
left_join(
  ...
  rename(Y10=value.x, YP=value.y, Year=date) |> 
  select(Year, Y10, YP) |> 
  arrange(Year) |>
  na.omit() |> 
  ggplot() +
  geom_line(aes(x=Year,y=Y10),size=1,color='black') + 
  geom_line(aes(x=Year,y=YP),size=1, color='red') + 
  theme_classic() +
  annotate("text",x=as.Date("2020-01-01"),y=7,label="Prime Rate", color="red") +
  annotate("text",x=as.Date("2005-06-01"),y=1,label="10 Y Treasury Rate", color="black") +
  ylab("Interest Rate") + 
...
#| fig-cap: "Real Disposable Personal Income Per Capita"
#| include: true
#| echo: false
#| warning: false
...
fredr::fredr(
  ...
  geom_line(size=1) + 
  theme_classic() +
  ylab("Chained 2012 Dollars (Thousands)") +
  scale_y_continuous(labels=scales::label_number(scale=.001))
